...
import { citySchoolGuide, SCHOOL_DIRECTORY_SOURCE, SCHOOL_DIRECTORY_URL, searchSchoolDirectory } from "./school-enrichment";
...
function PeopleView({ onOverlay }: { onOverlay: (overlay: Overlay) => void }) {
  ...
  const [flow, setFlow] = useState<{ kind: "menu" | "person" | "event" | "routine" | "profile-details" | "school-booking" | "school-document" | "school-document-detail" | "school-holiday" | "school-help"; scope?: CirclePersonScope; record?: CircleRecord; targetRecord?: CircleRecord; personId?: string; section?: ProfileMode } | null>(null);
  useEffect(() => {
  ...
  return <main className="view">
    ...
    {mode === "profiles" && <PeopleProfiles records={records} people={householdPeople} selectedId={selectedPersonId} onSelect={setSelectedPersonId} onAdd={() => setFlow({ kind: "person", scope: "household" })} onEdit={(record) => setFlow({ kind: "person", scope: "household", record })} onConfigure={(record, section) => setFlow({ kind: "profile-details", record, section })} onEvent={(personId) => setFlow({ kind: "event", personId })} onRoutine={(personId) => setFlow({ kind: "routine", personId })} onSchoolBooking={(record) => setFlow({ kind: "school-booking", record })} onSchoolDocument={(record) => setFlow({ kind: "school-document", record })} onSchoolDocumentDetail={(record, targetRecord) => setFlow({ kind: "school-document-detail", record, targetRecord })} onSchoolHoliday={(record) => setFlow({ kind: "school-holiday", record })} onSchoolHelp={(record) => setFlow({ kind: "school-help", record })} />}
...
function PeopleProfiles({ records, people, selectedId, onSelect, onAdd, onEdit, onConfigure, onEvent, onRoutine, onSchoolBooking, onSchoolDocument, onSchoolDocumentDetail, onSchoolHoliday, onSchoolHelp }: { records: CircleRecord[]; people: CircleRecord[]; selectedId: string | null; onSelect: (id: string) => void; onAdd: () => void; onEdit: (record: CircleRecord) => void; onConfigure: (record: CircleRecord, section: ProfileMode) => void; onEvent: (id: string) => void; onRoutine: (id: string) => void; onSchoolBooking: (record: CircleRecord) => void; onSchoolDocument: (record: CircleRecord) => void; onSchoolDocumentDetail: (record: CircleRecord, document: CircleRecord) => void; onSchoolHoliday: (record: CircleRecord) => void; onSchoolHelp: (record: CircleRecord) => void }) {
...
function SchoolProfileHub({ child, records, onConfigure, onBooking, onDocument, onDocumentDetail, onHoliday, onHelp }: { child: CircleRecord; records: CircleRecord[]; onConfigure: () => void; onBooking: () => void; onDocument: () => void; onDocumentDetail: (document: CircleRecord) => void; onHoliday: () => void; onHelp: () => void }) {
  ...
  const estimate = Number(booking?.payload.estimatedAmount || 0);
  ...
  return <section className="school-hub school-hub--rich school-hub--live">
    ...
    <div className="school-cost"><span className="tile-icon tile-icon--gold"><ReceiptText size={20} /></span><div><small>Estimation mensuelle</small><strong>{estimate ? `${estimate.toLocaleString("fr-FR", { minimumFractionDigits: 2 })} €` : "À calculer"}</strong><p>{counts.morning} matin{counts.morning > 1 ? "s" : ""} · {counts.lunch} midi{counts.lunch > 1 ? "s" : ""} · {counts.evening} soir{counts.evening > 1 ? "s" : ""}{booking?.payload.wednesdayBooked ? " · mercredi" : ""} par semaine</p></div><div className="school-cost__rule"><small>Tarification</small><b>{schoolText(school, "quotientBand", "Quotient à renseigner")}</b><em>{schoolText(school, "billingRule", guide?.billingRule || "Règle municipale à vérifier")}</em></div><button className="secondary-button" type="button" onClick={onBooking}>Voir le calcul</button></div>
    <div className="school-memory"><button type="button" onClick={onConfigure}><span className="tile-icon"><NotebookTabs size={18} /></span><span><small>Dans son sac</small><b>{schoolText(school, "bag", "À compléter")}</b></span><ChevronRight size={16} /></button><button type="button" onClick={onConfigure}><span className="tile-icon"><Phone size={18} /></span><span><small>Référent périscolaire</small><b>{schoolText(school, "afterSchoolContact", "À compléter")}</b></span><ChevronRight size={16} /></button><button type="button" onClick={onHoliday}><span className="tile-icon"><Trees size={18} /></span><span><small>Vacances</small><b>{holidays.length ? `${holidays.length} période${holidays.length > 1 ? "s" : ""} préparée${holidays.length > 1 ? "s" : ""}` : "Rien de préparé"}</b></span><ChevronRight size={16} /></button></div>
...
function SchoolSettingsFields({ values, setValues }: { values: Record<string, string>; setValues: React.Dispatch<React.SetStateAction<Record<string, string>>> }) {
  ...
  const choose = (school: SchoolDirectoryEntry) => {
  ...
  return <>
    <section className="school-assistant"><span className="school-assistant__mark"><Sparkles size={21} /></span><div><span className="eyebrow">Assistant École</span><h3>Partir des bonnes sources.</h3><p>Circle interroge l’annuaire public de l’Éducation nationale avec la ville et le nom saisis. Rien n’est enregistré avant votre validation.</p><div className="school-assistant__search"><label><span>Ville</span><input aria-label="Ville de l'école" value={values.city || ""} onChange={(event) => set("city", event.target.value)} placeholder="Rouen, Paris…" /></label><label><span>Nom ou quelques lettres</span><input aria-label="Rechercher une école" value={values.schoolName || ""} onChange={(event) => set("schoolName", event.target.value)} placeholder="Pépinières, Jules Ferry…" /></label><button className="primary-button primary-button--forest" type="button" disabled={searching || (values.city || "").trim().length < 2} onClick={() => void search()}>{searching ? <LoaderCircle className="spin" size={17} /> : <Search size={17} />} Rechercher</button></div>{searchError && <p className="school-assistant__error">{searchError}</p>}{results.length > 0 && <div className="school-results">{results.map((school) => <button type="button" key={school.uai} onClick={() => choose(school)}><span><b>{school.name}</b><small>{school.address} · {school.postalCode} {school.city}</small><em>{school.kind} · {school.publicPrivate} · UAI {school.uai}</em></span><ChevronRight size={17} /></button>)}</div>}{guide && <div className="school-guide-preview"><ShieldCheck size={17} /><span><b>{guide.portalName} identifié</b><small>{guide.services.join(" · ")}</small></span><a href={guide.informationUrl} target="_blank" rel="noreferrer">Vérifier <ExternalLink size={13} /></a></div>}</div></section>
    ...
    <div className="field-pair">{field("quotientBand", "Quotient ou tranche", "Ex. 851 - 1 150")}{field("reservationRule", "Délai de modification", "Ex. jusqu'à J-2")}</div>
...
function ProfileSchoolBookingFlow({ child, records, onClose }: { child: CircleRecord; records: CircleRecord[]; onClose: () => void }) {
  ...
  const [wednesdayArrival, setWednesdayArrival] = useState(String(existing?.payload.wednesdayArrival || "08:00"));
  const [wednesdayDeparture, setWednesdayDeparture] = useState(String(existing?.payload.wednesdayDeparture || "18:00"));
  ...
  const weekly = schoolWeek.reduce((sum, [day]) => sum + (schedule[day] || []).reduce((daySum, service) => daySum + numberFromSchool(school[`${service}Rate`]), 0), 0);
  const estimatedAmount = Math.round((weekly * 4.33 + (wednesdayBooked ? numberFromSchool(school.wednesdayRate) * 4 : 0)) * 100) / 100;
  const save = async () => {
    const payload = { personId: child.id, childName: child.title, schedule, wednesdayBooked, wednesdayArrival, wednesdayDeparture, estimatedAmount, portalUrl: school.portalUrl || "", source: school.portalName || "Saisie du foyer" };
    if (existing) await updateRecord(existing.id, { title: `Accueils scolaires de ${child.title}`, status: "planned", payload });
    else await addRecord({ kind: "school_booking", title: `Accueils scolaires de ${child.title}`, status: "planned", payload });
    setSaved(true);
  };
  return <OverlayFrame title={saved ? "Semaine enregistrée" : `Semaine d’école de ${child.title}`} eyebrow={saved ? "Agenda et estimation mis à jour" : "Réservations du foyer"} onClose={onClose} wide>{saved ? <div className="flow-success"><span><Check size={25} /></span><h3>La semaine est claire.</h3><p>Circle a enregistré vos choix. La réservation effective reste à confirmer sur le portail officiel de votre ville.</p>{Boolean(school.portalUrl) && <a className="secondary-button" href={String(school.portalUrl)} target="_blank" rel="noreferrer">Ouvrir {schoolText(school, "portalName", "le portail famille")} <ExternalLink size={15} /></a>}</div> : <div className="school-booking-flow school-booking-flow--live"><div className="booking-guidance"><CalendarClock size={20} /><span><b>Choisissez uniquement les accueils nécessaires.</b><small>{schoolText(school, "reservationRule", "Vérifiez ensuite les délais auprès de la ville.")}</small></span></div><div className="booking-table"><header><span>Jour</span><span>Matin</span><span>Midi</span><span>Soir</span></header>{schoolWeek.map(([day, label]) => <div key={day}><b>{label}</b>{(["morning", "lunch", "evening"] as SchoolService[]).map((service) => <button key={service} aria-label={`${label} · ${service === "morning" ? "matin" : service === "lunch" ? "midi" : "soir"}`} aria-pressed={Boolean(schedule[day]?.includes(service))} className={schedule[day]?.includes(service) ? "active" : ""} type="button" onClick={() => toggle(day, service)}>{schedule[day]?.includes(service) ? <Check size={15} /> : <Plus size={15} />}</button>)}</div>)}</div><button className={wednesdayBooked ? "booking-wednesday active" : "booking-wednesday"} type="button" onClick={() => setWednesdayBooked((value) => !value)}><span className="tile-icon tile-icon--gold"><Trees size={20} /></span><span><small>Mercredi · centre de loisirs</small><b>{wednesdayBooked ? "Prévu" : "Non prévu"}</b></span><strong>{school.wednesdayRate ? `${String(school.wednesdayRate)} €` : "Tarif à saisir"}</strong>{wednesdayBooked ? <Check size={18} /> : <Plus size={18} />}</button>{wednesdayBooked && <div className="field-pair"><label><span>Arrivée</span><input aria-label="Arrivée du mercredi" type="time" value={wednesdayArrival} onChange={(event) => setWednesdayArrival(event.target.value)} /></label><label><span>Départ</span><input aria-label="Départ du mercredi" type="time" value={wednesdayDeparture} onChange={(event) => setWednesdayDeparture(event.target.value)} /></label></div>}<div className="booking-price"><span><small>Estimation mensuelle</small><b>{estimatedAmount ? `${estimatedAmount.toLocaleString("fr-FR", { minimumFractionDigits: 2 })} €` : "Tarifs à renseigner"}</b></span><span><small>Tranche</small><b>{schoolText(school, "quotientBand", "À préciser")}</b></span><em>Estimation Circle, avant facture officielle</em></div></div>}<footer className="panel-footer"><button className="text-button" type="button" onClick={onClose}>{saved ? "Fermer" : "Annuler"}</button>{!saved && <button className="primary-button primary-button--forest" type="button" disabled={syncing} onClick={() => void save()}>{syncing && <LoaderCircle className="spin" size={17} />} Enregistrer dans Circle</button>}</footer></OverlayFrame>;
}

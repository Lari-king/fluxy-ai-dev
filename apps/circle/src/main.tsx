...
function Notifications({ onClose }: { onClose: () => void }) { const { records } = useCircleData(); const recent = records.slice(0, 5); return <OverlayFrame title="À suivre" eyebrow={`${recent.length || 3} nouveautés`} onClose={onClose}><div className="notification-list">{recent.length ? recent.map((record) => <Notification key={record.id} tone={record.status === "planned" ? "green" : record.status === "pending" ? "gold" : "coral"} icon={record.kind === "maintenance" ? <Wrench size={18} /> : record.kind === "document" ? <FileText size={18} /> : record.kind === "care_need" ? <HandHeart size={18} /> : <Check size={18} />} label={recordKindLabel(record.kind)} title={record.title} meta={record.status === "planned" ? "Planifié et partagé" : "Enregistré dans le foyer"} />) : <><Notification tone="coral" icon={<FileCheck2 size={18} />} label="À valider" title="Sortie à la ferme" meta="Autorisation avant mardi" /><Notification tone="gold" icon={<Clock3 size={18} />} label="En attente" title="Centre de loisirs" meta="Circle surveille la réponse" /><Notification tone="green" icon={<ReceiptText size={18} />} label="Nouveau" title="Facture d'électricité" meta="Analysée et rangée" /></>}</div><footer className="panel-footer"><button className="text-button" type="button" onClick={onClose}>Tout marquer comme vu</button><button className="primary-button primary-button--forest" type="button" onClick={onClose}>C'est noté</button></footer></OverlayFrame>; }

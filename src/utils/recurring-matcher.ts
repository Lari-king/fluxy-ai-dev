...
/**
 * Nettoie les descriptions bancaires "polluées"
 * Ex: "CB NETFLIX 123456 7890 PARIS" -> "netflix"
 */
function normalizeDescription(desc: string): string {
  if (!desc) return '';
  return desc
    .toLowerCase()
    .replace(/[0-9]{4,}/g, '') // Supprime les longs numéros (cartes, identifiants)
    .replace(/\s(paris|lyon|marseille|london|lu|fr|es|de|uk)\s/g, ' ') // Supprime les villes/pays courants
    .replace(/[^a-z ]/g, '') // Garde uniquement les lettres
    .trim();
}

/**
 * Distance de Jaro-Winkler (meilleure que Levenshtein pour les noms courts de commerçants)
 */
...
/**
 * RECURRING MATCHER V5.1 - "ENGINE DE PRÉCISION"
...
export function findSimilarTransactions(
  ...
  const targetNorm = normalizeDescription(target.description);
  ...
  return allTransactions
    ...
    .map(txn => {
      ...
      const amountScore = amountDiff === 0 ? 100 : 
                          ...
                          Math.max(0, 100 - (amountDiff / (targetAmount * 0.15)) * 100);

      // 2. Score de Texte (Jaro-Winkler sur descriptions nettoyées)
      const descScore = jaroWinkler(targetNorm, normalizeDescription(txn.description)) * 100;

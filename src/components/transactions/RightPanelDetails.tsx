...
import { normalize } from '@/utils/ruleEngine';

/**
 * 🧠 INTELLIGENCE : Analyse contextuelle des règles
 * Calcule le contexte d'une règle par rapport à l'historique des transactions
 */
const getRuleContext = (rule: Rule, transaction: Transaction, allTransactions: Transaction[], categories: any[] = []) => {
  ...
  const relevantTransactions = allTransactions.filter(t => {
    ...
    if (rule.type === 'category_budget') {
      const normTxCat = normalize(t.category || '');
  
      // ──────────────────────────────────────────────
      // SOLUTION : on utilise TOUJOURS la catégorie parent de la règle
      // (car subCategory n'est pas stockée dans transaction.category)
      // ──────────────────────────────────────────────
  
      const targetCategory = cond.category || cond.subCategory || '';  // fallback sur subCategory si pas de category
      const normTarget = normalize(targetCategory);
  
      // Match direct sur la catégorie parent
      if (normTxCat === normTarget) return true;
  
      // OU la transaction est une sous-catégorie du parent ciblé
      const parentCat = categories.find(
        c => normalize(c.name) === normTarget || normalize(c.id) === normTarget
      );
  
      if (parentCat) {
        const txCatObj = categories.find(c => normalize(c.name) === normTxCat);
        if (txCatObj && txCatObj.parentId === parentCat.id) return true;
      }
    ...
    // Marchand (inchangé)
    if ((rule.type === 'merchant_frequency' || rule.type === 'merchant_amount') && cond.merchant) {
      return t.description?.toLowerCase().includes(cond.merchant.toLowerCase());
    }
  ...
  console.log('[BUDGET CONTEXT DEBUG]', {
    ruleName: rule.name || rule.type || 'sans nom',
    ruleSubCategory: cond.subCategory || '—',
    ruleCategory: cond.category || '—',
    txCategoryAttendue: cond.category || cond.subCategory || '—',
    nombreTransactionsTrouvees: relevantTransactions.length,
    premierExempleCategory: relevantTransactions[0]?.category || 'aucun',
    premierExempleAmount: relevantTransactions[0]?.amount || 'aucun',
    premierExempleDate: relevantTransactions[0]?.date || 'aucun',
    totalTrouvePourDebug: relevantTransactions.reduce((s, tx) => s + Math.abs(tx.amount || 0), 0)
  });
  
  // ──────────────────────────────────────────────
  // LOG DE DEBUG (gardé et amélioré pour plus de visibilité)
  // ──────────────────────────────────────────────
  console.log('[BUDGET CONTEXT DEBUG]', {
    ruleName: rule.name || rule.type || 'sans nom',
    ruleSubCategory: cond.subCategory || '—',
    ruleCategory: cond.category || '—',
    txCategoryAttendue: cond.subCategory || cond.category || '—',
    nombreTransactionsTrouvees: relevantTransactions.length,
    premierExempleCategory: relevantTransactions[0]?.category || 'aucun',
    premierExempleAmount: relevantTransactions[0]?.amount || 'aucun',
    premierExempleDate: relevantTransactions[0]?.date || 'aucun',
  ...
  // LOG DE DEBUG (gardé pour vérifier)
  console.log('[BUDGET CONTEXT DEBUG]', {
    ruleName: rule.name || rule.type || 'sans nom',
    ruleSubCategory: cond.subCategory || '—',
    ruleCategory: cond.category || '—',
  ...
  if (rule.type === 'category_budget') {
    ...
    const period = 'period' in cond ? (cond.period || 'monthly') : 'monthly';
    const limit = 'maxAmount' in cond ? (cond.maxAmount || 0) : 0;

    const txDate = new Date(transaction.date);

    // Filtrer par période (mois courant de la transaction affichée)
    const periodTransactions = relevantTransactions.filter(t => {
      // Sécurité : si pas de date ou date invalide → on skippe
      if (!t.date) {
        console.warn('[PERIODE FILTER] Transaction sans date ignorée', t.id);
        return false;
      }
    
      const d = new Date(t.date);
      if (isNaN(d.getTime())) {
        console.warn('[PERIODE FILTER] Date invalide ignorée', t.id, t.date);
        return false;
      }
    
      const txD = new Date(transaction.date);
      if (isNaN(txD.getTime())) {
        console.warn('[PERIODE FILTER] Date de la transaction sélectionnée invalide', transaction.id, transaction.date);
        return false;
      }
    
      // Filtre selon la période définie dans la règle
      if (period === 'daily') {
        return d.toDateString() === txD.toDateString();
      } 
      else if (period === 'weekly') {
        const weekStart = new Date(txD);
        weekStart.setDate(txD.getDate() - txD.getDay());
        const weekEnd = new Date(weekStart);
        weekEnd.setDate(weekStart.getDate() + 7);
        return d >= weekStart && d < weekEnd;
      } 
      else if (period === 'monthly') {
        return d.getFullYear() === txD.getFullYear() && d.getMonth() === txD.getMonth();
      } 
      else if (period === 'yearly') {
        return d.getFullYear() === txD.getFullYear();
      }
    
      // Période inconnue → on logge et on skippe
      console.warn('[PERIODE FILTER] Période inconnue', period);
      return false;
    });
    
    // ──────────────────────────────────────────────
    // LOG DE DEBUG TRÈS UTILE pour comprendre pourquoi le total est incorrect
    // ──────────────────────────────────────────────
    console.log('[PERIODE FILTER DEBUG]', {
      ruleName: rule.name || rule.type || 'sans nom',
      moisTransactionSélectionnée: txDate.toLocaleString('fr-FR', { month: 'long', year: 'numeric' }),
      périodeRègle: period || 'non défini',
      nombreTransactionsDansPériode: periodTransactions.length,
      totalCalcule: periodTransactions.reduce((s, t) => s + Math.abs(t.amount || 0), 0).toFixed(2) + ' €',
      transactionsDuMois: periodTransactions.map(t => ({
        id: t.id,
        date: t.date,
        category: t.category,
        amount: t.amount,
    ...
    const totalSpent = periodTransactions.reduce((sum, t) => sum + Math.abs(t.amount || 0), 0);
  ...
  // ──────────────────────────────────────────────
  // FRÉQUENCE MARCHAND (exemple – tu peux compléter si besoin)
  // ──────────────────────────────────────────────
  if (rule.type === 'merchant_frequency') {
    const period = 'frequencyPeriod' in cond ? (cond.frequencyPeriod || 'monthly') : 'monthly';
    const txDate = new Date(transaction.date);
    ...
    return {
      ...
      limit: 'maxFrequency' in cond ? (cond.maxFrequency || 0) : 0,
      overage: periodTransactions.length - ('maxFrequency' in cond ? cond.maxFrequency || 0 : 0)
    };
  }
...
export function RightPanelDetails({ 
  ...
  categories = [],
  people = [],
  // Suppression de allTransactions des props car on le prend du contexte
}: RightPanelDetailsProps) {
  
  // --- HOOKS ---
  // On récupère les rules et transactions depuis DataContext, les violations depuis RulesContext
  const { rules, transactions: allTransactions } = useData();
  ...
  // 🆕 AJOUTER CES 3 VARIABLES (Elles manquent dans votre fichier actuel)
  ...
  const hasChildTransactions = useMemo(() => {
    if (!(transaction as any)?.childTransactionIds) return false;
    const ids = (transaction as any).childTransactionIds;
    return Array.isArray(ids) && ids.length > 0;
  }, [transaction]);

  const childCount = useMemo(() => {
    return (transaction as any)?.childTransactionIds?.length || 0;
  }, [transaction]);

  const matchedRules = useMemo(() => {
    if (!transaction || !rules) return [];
  
    return rules
      .filter(rule => rule.enabled)
      .map(rule => {
        // Évaluation de la règle sur la transaction sélectionnée
        const result = evaluateRule(rule, transaction);
  
        // Si pas de match, on saute cette règle
        if (!result.matches) return null;
  
        // Calcul du contexte (budget utilisé, dépassement, etc.)
        // → on passe allTransactions et categories (définis via useData)
        const context = getRuleContext(rule, transaction, allTransactions, categories);
  ...
  }, [transaction, rules, allTransactions, categories]);
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-white/70 leading-relaxed">
                          {getRuleExplanation(rule, reason)}
                        </div>
                      </div>
                    </div>
                  </div>
 ...
 */
/**
 * 📝 Génère une explication lisible de la règle déclenchée
 * Utilise 'any' pour les conditions afin de contourner les strictions de l'union de types TS
 */
function getRuleExplanation(rule: any, reason?: string): string {
  ...
  switch (rule.type) {
    ...
    case 'merchant_frequency':
      return `Surveillance active sur ${conditions.merchant || 'ce marchand'}. ${
        conditions.merchantMaxAmount 
          ? `Seuil : ${formatCurrency(conditions.merchantMaxAmount)}.`
          : 'Analyse des habitudes de dépense.'
      }`;

    case 'category_budget':
    case 'category_limit':
      return `Limite de budget pour la catégorie "${conditions.category || 'sélectionnée'}".`;

    case 'time_range':
      const timeStr = conditions.startTime && conditions.endTime 
        ? `entre ${conditions.startTime} et ${conditions.endTime}`
        : 'sur une plage horaire spécifique';
      
      // ✅ Appel à getPeriodLabel pour corriger l'erreur de variable inutilisée
      const period = getPeriodLabel(conditions.timeRangePeriod);
      
      return `Transaction détectée ${timeStr}. ${
        conditions.timeRangeMaxAmount
          ? `Limite de ${formatCurrency(conditions.timeRangeMaxAmount)} ${period}.`
          : ''
      }`;

    case 'recurring_variance':
      return `Abonnement "${conditions.recurringDescription || 'détecté'}" sous surveillance. ${
        conditions.maxVariancePercent 
          ? `Alerte si le prix varie de plus de ${conditions.maxVariancePercent}%.`
          : ''
      }`;

    case 'person_flow':
      return `Flux financier associé à "${conditions.personName || 'une relation'}". ${
        conditions.minAmount 
          ? `Surveillance des montants supérieurs à ${formatCurrency(conditions.minAmount)}.`
          : ''
      }`;

    default:
      // Si une raison spécifique est fournie par le moteur de règle, on l'affiche
      return reason || 'Cette transaction correspond aux critères de sécurité définis.';
  }
}

/**
 * 📅 Retourne le label de période en français
 * (Placée ici pour s'assurer qu'elle est accessible par getRuleExplanation)
 */

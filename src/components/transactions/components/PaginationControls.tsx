/**
 * 📄 PAGINATION CONTROLS - DESIGN SYSTEM HARMONISÉ
 * 
 * Design cohérent avec LeftPanel et Cards :
 * - Tailles text-xs/text-sm
 * - Colors cyan/purple
 * - Spacing p-2/p-3
    ...
    <div className="flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10">
        <div className="flex flex-col items-end">
          ...
          <span className={`text-sm font-mono font-bold ${totalAmount >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            {formatAmount(totalAmount)}
          </span>
        </div>
        <div className={`p-2 rounded-lg ${totalAmount >= 0 ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
           <Calculator className={`w-4 h-4 ${totalAmount >= 0 ? 'text-green-400' : 'text-red-400'}`} />
        </div>
      </div>

    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
          ...
          <span>{typeConfig?.label || 'Règle'}</span>
        </div>
...
export function RulesLeftPanel({
  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-thin bg-black border-r border-white/10 w-[440px]">
        ...
        <div className="space-y-3">
          {rules.length === 0 ? (
            <div className="text-center py-10 border border-dashed border-white/10 rounded-xl">
              <p className="text-sm text-white/40">Aucune règle définie</p>
            </div>
          ) : (
...
export function RulesDiagnostic() {
  const { rules: dataRules, loading } = useData();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[9999] p-4 bg-black border border-cyan-500 rounded-lg text-[10px] font-mono text-cyan-400 max-w-xs shadow-2xl">
      ...
      <div className="space-y-1">
        <p>DataContext Rules: <span className="text-white">{dataRules?.length || 0}</span></p>
        <p>Loading Status: <span className="text-white">{loading ? 'EN COURS' : 'OK'}</span></p>
      </div>
    </div>
  );
}
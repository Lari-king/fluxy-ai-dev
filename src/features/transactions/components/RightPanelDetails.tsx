 ...
 */
const getRuleContext = (rule: any, transaction: Transaction, allTransactions: Transaction[]) => {
...
interface RightPanelDetailsProps {
  ...
  rules?: any[]; // 🆕 Liste des règles pour évaluation
  onClose: () => void;
...
export function RightPanelDetails({ 
  ...
  const matchedRules = useMemo(() => {
    ...
    return rules
      ...
      .map(rule => {
        // Évaluation basique inline (à remplacer par votre evaluateRule si disponible)
        const matches = evaluateRuleBasic(rule, transaction);
      ...
      .filter(Boolean) as Array<{ rule: any; context: any }>;
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-white/70 leading-relaxed">
                          {getRuleExplanation(rule, transaction)}
                        </div>
                      </div>
                    </div>
                  </div>
 ...
 */
function getRuleExplanation(rule: any, transaction: Transaction): string {
  const conditions = rule.conditions;
  const amount = Math.abs(transaction.amount);

  switch (rule.type) {
    case 'keyword_detection':
      return `Le libellé "${transaction.description}" contient un mot-clé surveillé par cette règle.`;
    ...
    default:
      return 'Cette transaction correspond aux critères de la règle.';
  }
}
...
/**
 * 🎯 Évaluation basique des règles (simplifié)
 * À remplacer par votre vraie fonction evaluateRule si disponible
 */
function evaluateRuleBasic(rule: any, transaction: Transaction): boolean {
  // Implémentation simplifiée - à adapter selon vos besoins
  if (!rule.enabled) return false;
  
  // Exemple basique pour category_budget
  if (rule.type === 'category_budget') {
    return transaction.category === rule.conditions.category;
  }
